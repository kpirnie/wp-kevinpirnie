 ...
 *
 */
if ( ! class_exists( 'KPT_FW_Field_dimensions' ) ) {
  class KPT_FW_Field_dimensions extends KPT_FW_Fields {
    ...
    public function render() {
      ...
      $args = wp_parse_args( $this->field, array(
        ...
        'width_placeholder'  => esc_html__( 'width', 'kpt_fw' ),
        'height_placeholder' => esc_html__( 'height', 'kpt_fw' ),
        'width'              => true,
        'height'             => true,
        'unit'               => true,
        'show_units'         => true,
      ...
      if ( ! empty( $args['width'] ) ) {
        ...
        echo ( ! empty( $args['width_icon'] ) ) ? '<span class="kpt_fw--label kpt_fw--icon">'. $args['width_icon'] .'</span>' : '';
        echo '<input type="number" name="'. esc_attr( $this->field_name( '[width]' ) ) .'" value="'. esc_attr( $value['width'] ) .'"'. $placeholder .' class="kpt_fw-input-number'. esc_attr( $is_unit ) .'" step="any" />';
        echo ( ! empty( $unit ) ) ? '<span class="kpt_fw--label kpt_fw--unit">'. esc_attr( $args['units'][0] ) .'</span>' : '';
        echo '</div>';
      }
      ...
      if ( ! empty( $args['height'] ) ) {
        ...
        echo '<div class="kpt_fw--input">';
        echo ( ! empty( $args['height_icon'] ) ) ? '<span class="kpt_fw--label kpt_fw--icon">'. $args['height_icon'] .'</span>' : '';
        echo '<input type="number" name="'. esc_attr( $this->field_name( '[height]' ) ) .'" value="'. esc_attr( $value['height'] ) .'"'. $placeholder .' class="kpt_fw-input-number'. esc_attr( $is_unit ) .'" step="any" />';
        echo ( ! empty( $unit ) ) ? '<span class="kpt_fw--label kpt_fw--unit">'. esc_attr( $args['units'][0] ) .'</span>' : '';
        echo '</div>';
      }
