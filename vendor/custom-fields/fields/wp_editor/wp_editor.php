 ...
 *
 */
if ( ! class_exists( 'KPT_FW_Field_wp_editor' ) ) {
  class KPT_FW_Field_wp_editor extends KPT_FW_Fields {
    ...
    public function render() {
      ...
      echo ( kpt_fw_wp_editor_api() ) ? '<div class="kpt_fw-wp-editor" data-editor-settings="'. esc_attr( json_encode( $editor_settings ) ) .'">' : '';

      echo '<textarea name="'. esc_attr( $this->field_name() ) .'"'. $this->field_attributes( $attributes ) . $editor_height .'>'. $this->value .'</textarea>';

      echo ( kpt_fw_wp_editor_api() ) ? '</div>' : '';
    ...
    public function enqueue() {
      ...
      if ( kpt_fw_wp_editor_api() && function_exists( 'wp_enqueue_editor' ) ) {
    ...
    public function setup_wp_editor_media_buttons() {
      ...
      echo 'var kpt_fw_media_buttons = '. json_encode( $media_buttons ) .';';
      echo '</script>';
    ...
    public function setup_wp_editor_settings() {
      ...
      if ( kpt_fw_wp_editor_api() && class_exists( '_WP_Editors') ) {

        $defaults = apply_filters( 'kpt_fw_wp_editor', array(
          'tinymce' => array(
            'wp_skip_init' => true
          ),
        ) );

        $setup = _WP_Editors::parse_settings( 'kpt_fw_wp_editor', $defaults );

        _WP_Editors::editor_settings( 'kpt_fw_wp_editor', $setup );
