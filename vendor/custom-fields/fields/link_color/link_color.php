 ...
 *
 */
if ( ! class_exists( 'KPT_FW_Field_link_color' ) ) {
  class KPT_FW_Field_link_color extends KPT_FW_Fields {
    ...
    public function render() {
      ...
      $color_props = array(
        'color'    => esc_html__( 'Normal', 'kpt_fw' ),
        'hover'    => esc_html__( 'Hover', 'kpt_fw' ),
        'active'   => esc_html__( 'Active', 'kpt_fw' ),
        'visited'  => esc_html__( 'Visited', 'kpt_fw' ),
        'focus'    => esc_html__( 'Focus', 'kpt_fw' )
      );
      ...
      foreach ( $color_props as $color_prop_key => $color_prop_value ) {
        ...
        if ( ! empty( $args[$color_prop_key] ) ) {
          ...
          echo '<div class="kpt_fw--left kpt_fw-field-color">';
          echo '<div class="kpt_fw--title">'. esc_attr( $color_prop_value ) .'</div>';
          echo '<input type="text" name="'. esc_attr( $this->field_name( '['. $color_prop_key .']' ) ) .'" value="'. esc_attr( $value[$color_prop_key] ) .'" class="kpt_fw-color"'. $default_attr . $this->field_attributes() .'/>';
          echo '</div>';
