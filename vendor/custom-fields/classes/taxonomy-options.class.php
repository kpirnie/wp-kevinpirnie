 ...
 *
 */
if ( ! class_exists( 'KPT_FW_Taxonomy_Options' ) ) {
  class KPT_FW_Taxonomy_Options extends KPT_FW_Abstract{
    ...
    public function render_taxonomy_form_fields( $term ) {
      ...
      $errors    = ( ! empty( $term_id ) ) ? get_term_meta( $term_id, '_kpt_fw_errors_'. $this->unique, true ) : array();
      $errors    = ( ! empty( $errors ) ) ? $errors : array();
      $class     = ( $this->args['class'] ) ? ' '. $this->args['class'] : '';

      if ( ! empty( $errors ) ) {
        delete_term_meta( $term_id, '_kpt_fw_errors_'. $this->unique );
      }

      wp_nonce_field( 'kpt_fw_taxonomy_nonce', 'kpt_fw_taxonomy_nonce'. $this->unique );

      echo '<div class="kpt_fw kpt_fw-taxonomy kpt_fw-show-all kpt_fw-onload kpt_fw-taxonomy-'. esc_attr( $classname ) .'-fields '. esc_attr( $class ) .'">';
        ...
        if ( $taxonomy === $this->taxonomy ) {
          ...
          if ( ! empty( $section['fields'] ) ) {
            foreach ( $section['fields'] as $field ) {
              ...
              KPT_FW::field( $field, $this->get_meta_value( $field, $term_id ), $this->unique, 'taxonomy' );
    ...
    public function save_taxonomy( $term_id ) {
      ...
      $data     = array();
      $errors   = array();
      $noncekey = 'kpt_fw_taxonomy_nonce'. $this->unique;
      $nonce    = ( ! empty( $_POST[ $noncekey ] ) ) ? sanitize_text_field( wp_unslash( $_POST[ $noncekey ] ) ) : '';
      $taxonomy = ( ! empty( $_POST[ 'taxonomy' ] ) ) ? sanitize_text_field( wp_unslash( $_POST[ 'taxonomy' ] ) ) : '';

      if ( ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) || ! wp_verify_nonce( $nonce, 'kpt_fw_taxonomy_nonce' ) ) {
        return $term_id;
      }
      ...
      $data = apply_filters( "kpt_fw_{$this->unique}_save", $data, $term_id, $this );

      do_action( "kpt_fw_{$this->unique}_save_before", $data, $term_id, $this );
      ...
      if ( empty( $data ) ) {
        ...
        if ( ! empty( $errors ) ) {
          update_term_meta( $term_id, '_kpt_fw_errors_'. $this->unique, $errors );
        }

      }

      do_action( "kpt_fw_{$this->unique}_saved", $data, $term_id, $this );

      do_action( "kpt_fw_{$this->unique}_save_after", $data, $term_id, $this );
