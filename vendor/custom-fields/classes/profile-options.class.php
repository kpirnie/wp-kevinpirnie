 ...
 *
 */
if ( ! class_exists( 'KPT_FW_Profile_Options' ) ) {
  class KPT_FW_Profile_Options extends KPT_FW_Abstract{
    ...
    public function render_profile_form_fields( $profileuser ) {
      ...
      $errors     = ( ! empty( $profile_id ) ) ? get_user_meta( $profile_id, '_kpt_fw_errors_'. $this->unique, true ) : array();
      $errors     = ( ! empty( $errors ) ) ? $errors : array();
      $class      = ( $this->args['class'] ) ? ''. $this->args['class'] : '';

      if ( ! empty( $errors ) ) {
        delete_user_meta( $profile_id, '_kpt_fw_errors_'. $this->unique );
      }

      echo '<div class="kpt_fw kpt_fw-profile-options kpt_fw-onload'. esc_attr( $class ) .'">';

      wp_nonce_field( 'kpt_fw_profile_nonce', 'kpt_fw_profile_nonce'. $this->unique );
      ...
      foreach ( $this->sections as $section ) {
        ...
        if ( ! empty( $section['fields'] ) ) {
          ...
          foreach ( $section['fields'] as $field ) {
            ...
            KPT_FW::field( $field, $this->get_meta_value( $profile_id, $field ), $this->unique, 'profile' );
    ...
    public function save_profile( $user_id ) {
      ...
      $data     = array();
      $errors   = array();
      $noncekey = 'kpt_fw_profile_nonce'. $this->unique;
      $nonce    = ( ! empty( $_POST[ $noncekey ] ) ) ? sanitize_text_field( wp_unslash( $_POST[ $noncekey ] ) ) : '';

      if ( ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) || ! wp_verify_nonce( $nonce, 'kpt_fw_profile_nonce' ) ) {
        return $user_id;
      }
      ...
      $data = apply_filters( "kpt_fw_{$this->unique}_save", $data, $user_id, $this );

      do_action( "kpt_fw_{$this->unique}_save_before", $data, $user_id, $this );
      ...
      if ( empty( $data ) ) {
        ...
        if ( ! empty( $errors ) ) {
          update_user_meta( $user_id, '_kpt_fw_errors_'. $this->unique, $errors );
        }

      }

      do_action( "kpt_fw_{$this->unique}_saved", $data, $user_id, $this );

      do_action( "kpt_fw_{$this->unique}_save_after", $data, $user_id, $this );
